<script lang="ts">  import { Button } from "$lib/components/ui/button";
  ...
  onMount(async () => {
    ...
    unlistenLlm = await listen("llm-progress", (event) => {
      llmProgress = typeof event.payload === "string" ? event.payload : JSON.stringify(event.payload, null, 2);
...
<div class="container flex flex-col gap-6 p-4">
  ...
  <Card.Root>
    <Card.Header>
      ...
      <Card.Description>
        Configure how the application generates summaries and meeting names. You can use an external API like Ollama, or fall back to the built-in Kalosm model.
      </Card.Description>
    </Card.Header>    <Card.Content class="space-y-4">
      <div class="space-y-2">
      ...
      {:else}
        <div class="p-3 bg-muted rounded-lg">
          <p class="text-sm text-muted-foreground">
            Using built-in Kalosm model (Phi-3.5 Mini). This will download the model on first use (~2GB).
          </p>
        </div>
      {/if}    </Card.Content>
    ...
    <div class="mt-4 p-4 bg-muted rounded-lg">
      ...
      {#if isTestingLlm}
        <div class="mt-2 flex items-center gap-2">
          ...
          <span class="text-sm text-muted-foreground">Testing LLM connection...</span>
        </div>
      {/if}
    </div>
  {/if}
